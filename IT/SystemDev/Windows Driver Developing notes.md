...
`MmProbeAndLockPages` - тестирует и блокирует выгрузку в файл подкачки указанную виртуальную память, делая буфер в драйвере резидентом физической памяти. Работает на APC уровне для PAGED памяти. На DPC уровне работает для NON-PAGED памяти.
