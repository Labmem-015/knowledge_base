...
# IRP 
## Method
Здесь задаётся способ передачи данных между вызывающим `DeviceIoControl` и драйвером-обработчиком IRP. Метода всего три:
- `METHOD_BUFFERED` предоставляет невыгружаемый системный буфер для записи и чтения. Он копирует данные из пользовательского буфера в системный. Подходит для маленьких порций данных.
- `METHOD_IN_DIRECT` или `METHOD_OUT_DIRECT` позволяет ОС блокировать пользовательский буфер в памяти. Затем создаётся MDL, который идентифицирует эту память, и она передаётся в стек драйверов. Доступ к ним производится через MDL.
- `METHOD_NEITHER` передаёт виртуальный адрес пользовательского буфера. Буфер доступен только из драйверов, которые работают в контексте потока приложения.
Для первых двух типов указатель на буферы можно получить из структуры `IO_STACK_LOCATION`: `Irp->AssociatedIrp.SystemBuffer`. Аналогично и размеры буфера: `Parameters.DeviceIoControl.InputBufferLength` и `Parameters.DeviceIoControl.OutputBufferLength`. Для Direct I/O указатель на MDL можно получить в `Irp->MdlAddress`.

---
# MDL
Memory Descriptor List, который связывает виртуальный адрес с физическими страницами памяти, описываемого MDL.
`IoAllocateMdl` аллоцирует память под структуру MDL.
`MmProbeAndLockPages` - (заморозка физических страниц указанной виртуальной памяти) тестирует и блокирует выгрузку в файл подкачки указанную виртуальную память, делая буфер в драйвере резидентом физической памяти. Работает на APC уровне для PAGED памяти. На DPC уровне работает для NON-PAGED памяти.
`MmMapLockedPagesSpecifyCache` производит сопоставление физической памяти (MDL дескриптор) с новой виртуальной (return value). При этом можно указать способ кэширования.

Как я понял: через `IoAllocateMdl` мы готовимся мапить файл. Через `MmProbeAndLockPages` мы производим отображение в физическую память. Через `MmMapLockedPagesSpecifyCache` мы создаём второй виртуальный адрес (ещё один псевдоним для физической памяти).
...
Display DWORD. L1 означает один адрес.
Получение смещения по индексу в SSDT:
```kd
dd nt!KiServiceTable + (0x55 * 4) L1
```
Получение адреса функции по значению смещения:
```kd
u nt!KiServiceTable + (0x08ac6c07 >> 4)
```
Display Pointers and Symbols - используется для отображения содержимого памяти в виде списка **указателей** и соответствующих им **символьных имен**:
```kd
dpc <address>
```
...
![[SSDT.png]]
Таблица хранит смещения и адреса функций, относительно начала таблицы.
Чтобы получить доступ к адресу функции нужно следующее:
- По индексу в таблице получить 32 битное смещение.
- Сделать побитовый сдвиг вправо на 4 бита для этого смещения.
- Получить адрес функции по финальному смещению от начала таблицы.
Но последние 4 бита в значениях этих смещений обозначают количество аргументов системной функции. Поэтому для получения нужного смещения, необходимо выполнить побитовый сдвиг вправо. Выходит такая формула:
```cpp
