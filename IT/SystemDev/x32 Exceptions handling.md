...
10) ыва
11) ыва
12) [Про /SAFESEH](https://learn.microsoft.com/en-us/cpp/build/reference/safeseh-image-has-safe-exception-handlers?view=msvc-170)
...
> 
 [Источник](https://ru.stackoverflow.com/questions/933731/%D0%97%D0%B0%D1%87%D0%B5%D0%BC-%D0%BD%D1%83%D0%B6%D0%B5%D0%BD-%D1%84%D0%BB%D0%B0%D0%B3-ehsc-%D0%B4%D0%BB%D1%8F-msvc-%D0%BA%D0%BE%D0%BC%D0%BF%D0%B8%D0%BB%D1%8F%D1%82%D0%BE%D1%80%D0%B0)
...
## x32 reversing
Я заметил, что в C++ исключениях в exception registration отличаются обработчики на каждом узле. По итогу они скорее всего являются некими обёртками, которые ведут на один и тот же обработчик.
## x64 reversing
...
На этапе отладки выяснилось, что Win7 x32 имеет нулевые значения на `FS:[0x04]` и `FS:[0x08]`, в которых должны содержаться адреса базы стека и его предела. Они нужны для валидации указателя на список обработчиков исключений (`SEH_REGISTRATION`). Нужно понять, что представляет собой процесс и потоки в ядре и как выглядит стек в режим ядра. 
## Общие сведения об ассемблере
