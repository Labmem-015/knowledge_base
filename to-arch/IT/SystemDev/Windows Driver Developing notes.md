...
Для изменения уровня запроса прерывания можно использовать функции `KeRaiseIrql` `KeLowerIrql`. При повышении уровня, рекомендуется перед возвращением функции понижать этот уровень, чтобы это снизить риски падения системы.
